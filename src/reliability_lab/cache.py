...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        ...
        def tokens(text: str) -> list[str]:
            ...
            ngrams = [normalized[index : index + 3] for index in range(max(0, len(normalized) - 2))]
            return words + ngrams

        left, right = Counter(tokens(a)), Counter(tokens(b))
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if best_query is not None and best_response is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_query):
                self.false_hit_log.append(
                    {"query": query, "cached_key": best_query, "score": best_score, "reason": "date_or_number_mismatch"}
                )
                return None, best_score
